...
class SkyWars extends PluginBase implements Listener {
    ...
    public function onEnable() {
        ...
        $this->getServer()->getCommandMap()->register("VampireZ", $this->commands[] = new SkyWarsCommand($this));
    }
    ...
    public function onChat(PlayerChatEvent $event) {
        ...
        switch ($args[0]) {
            case "help":
                $player->sendMessage("§a> VampireZ устаноаочные команды(1/1):\n".
                "§7help : Помощь по командам\n" .
                "§7slots <количество> : Установить слоты\n".
                "§7level <название> : Установить мир\n".
                "§7spawn <слот> : Установить точку спавна\n".
                "§7breaking <true/false> : Включить разрушение блоков на арене\n".
                "§7joinsign : Установить входную табличку\n".
                "§7savelevel : Сохранить карту\n".
                "§7enable : Активировать арену");
                break;
            case "slots":
                if(!isset($args[1])) {
            ...
            case "spawn":
                ...
                break;
            case "breaking":
            	if(in_array($args[1], array("true", "false","t","f"))){
            		if(($args[1] == "true")or($args[1] == "t")){
            			$player->sendMessage("§a> На арене теперь можно рушить блоки!");
            			$arena->data["breaking"] = true;
            			break;
            		}else{
            			$player->sendMessage("§a> На арене теперь нельзя рушить блоки!");
            			$arena->data["breaking"] = false;
            			break;
            		}
            	}else{
            		$player->sendMessage("§a> Используйте: breaking <true/false>!");
            		break;
            	}
            case "joinsign":
        ...
        if(isset($this->setupData[$player->getName()])) {
            switch ($this->setupData[$player->getName()]) {
                case 0:
                    ...
                    $player->sendMessage("§a> Входная табличка обновлена!");
                    unset($this->setupData[$player->getName()]);
                    $event->setCancelled(\true);
                    break;
            }
